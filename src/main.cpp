...
map<char*, int, POSCompare> POS_map;//—оответствие ЌазваниеPOS - »ндекс
//set<int> truePOS;//ћножество индексов POS, которые учитываютс€ в анализе
...
"For instance, if words Уcrocodile, alligator, house, and buildingФ were given as input the program will try to return pairs Уcrocodile,alligatorФ and Уhouse,buildingФ among all possible combinations.\n");
	printf("\n"
...
"\to Ц Gloss overlap measure equal to number of common words in the definitions of two words.\n"
"\tc Ц Cosine between bag-of-word vectors build from definitions of respective of two words\n"
"\tk Ц Karaulov's semantic similarity measure.\n");
	printf("\n"
...
"K - number of nearest-neighbors, default K = 2\n");
	printf("\n"
"T1, T2, T3 Ц parameters of Karaulov's semantic similarity measure, default T1 = 2, T1 = 1, T3 = 6.\n");
	printf("\n\nAuthors:\n");
...
//	globalArgs.outputFile = "result.dat";
	...
	while( opt != -1 ) 
	{
		...
		case 'S'://ћетод определени€ схожести
			...
			break;
		case 'M'://јлгоритм вычислени€ семантических отношений
			switch(*optarg)
			{
			case '1':
			...
			break;
		case 'K':// оличество отношений дл€ каждого из слов
			globalArgs.K = atoi(optarg);
			break;
		case 'T'://3 параметра дл€ метода караулова
			if (optind + 1 < argc)
			{
